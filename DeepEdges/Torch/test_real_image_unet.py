...
class EdgesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        Y = curI
        B = cv2.Canny(np.uint8(255*Y), 0.4, 0.7)
        B = B.astype('float32') / 255
        sz = Y.shape
        Y = np.reshape(Y,(1,sz[0],sz[1]))
        Y = np.concatenate((Y,Y,Y),0)
        B = np.reshape(B,(1,sz[0],sz[1]))
...
dataset = EdgesDataset('../../FinalImages')
train_size = 0
...
def test_model(model, num_epochs):
    for epoch in range(num_epochs):
        ...
        for phase in ['test']:
            ...
            epoch_samples = 0
            totalLoss = 0
            for inputs, labels, sz in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    plt.figure()
                    plt.subplot(1,2,1)
                    plt.imshow(reverse_transform(inputs[:,:,0:sz[0],0:sz[1]]))
                    plt.subplot(1,2,2)
                    plt.imshow(reverse_transform(outputs[:,:,0:sz[0],0:sz[1]]))
                    #plt.subplot(1,3,3)
